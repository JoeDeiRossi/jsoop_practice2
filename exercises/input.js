...
class Input{
	...
	//setPattern saves a regex pattern into the input object

	//arguments: pattern (a regular expression.  if you don't know what it is, you will learn it soon)
	//returns: nothing
	//saves the given pattern into the input object
	setPattern(pattern){
		this.regex = pattern;
	}
	...
	test(){
		var targetInputVal = this.targetInput.val();
		var results = {};
		var passedPatternTest;
		var passedRangeTest;

		if (this.regex.test(targetInputVal)) {
			passedPatternTest = true;
		} else {
			passedPatternTest = false;
		}

		if (this.range.min && this.range.max) {
			if (this.range.min <= targetInputVal && this.range.max >= targetInputVal) {
		...
		if (passedPatternTest === false) {
			results.result = false;
			results.error = 'pattern';
		} else if (passedRangeTest === false) {
			results.result = false;
			results.error = 'range';
		} else {
			results.result = true;
	...
	showError(errorMessage){
		var inputLeft = this.targetInput.position().left;
		var inputTop = this.targetInput.position().top;
		var inputHeight = this.targetInput.height();
		this.messageElement = $('<div>').css({'left': inputLeft, 'top': inputTop + inputHeight}).addClass('inputError').text(errorMessage);
		this.targetInput.parent().append(this.messageElement);
	}
	/*
